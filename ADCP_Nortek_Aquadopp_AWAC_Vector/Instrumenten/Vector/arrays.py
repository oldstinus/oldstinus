...
class VectorOnlyApp(tk.Tk):
    ...
    def _x_values(self, df2: pd.DataFrame) -> Tuple[np.ndarray | List[pd.Timestamp], str]:
        if self.x_cb.get() == "Sample_Index":
            return df2["Sample_Index"].to_numpy(dtype=float), "Sample index"
        # Type-safe: altijd via pd.to_datetime
        return pd.to_datetime(df2.index).to_pydatetime(), "Tijd"
