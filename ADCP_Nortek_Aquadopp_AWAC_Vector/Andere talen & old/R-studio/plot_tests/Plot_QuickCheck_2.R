...
#PLOT 1 - waterdepth
# AQP - waves
plot(dat_AQP_wave_lowPT[[1]]$Timestamp[ind_event_1s], dat_AQP_wave_lowPT[[1]]$WaterDepth[ind_event_1s], type = 'l', ylim = c(5.2,5.5))
lines(dat_AQP_wave_lowPT_1m[[1]]$Timestamp[ind_event_1m], dat_AQP_wave_lowPT_1m[[1]]$WaterDepth[ind_event_1m], lty = 2, col = "black")
# AQP - tide
lines(AQP_tidePT[[1]]$Timestamp[ind_event_5m], AQP_tidePT[[1]]$WaterDepth[ind_event_5m], col = "orange")
# Vector
...
#PLOT 2 - velocities
# AQP - waves
plot(dat_AQP_wave_lowPT[[1]]$Timestamp[ind_event_1s], dat_AQP_wave_lowPT[[1]]$Vel[ind_event_1s], type = 'l', ylim = c(-0.2,0.6))
lines(dat_AQP_wave_lowPT_1m[[1]]$Timestamp[ind_event_1m], dat_AQP_wave_lowPT_1m[[1]]$Vel[ind_event_1m], type = 'l', lty  = 2)
...
# Vector
#lines(dat_AQP_wave_lowPT[[1]]$Timestamp[ind_event_1s], Vec_1s[[1]]$Vel[ind_event_1s], col = "blue")
lines(dat_AQP_wave_lowPT[[1]]$Timestamp[ind_event_1s], Vec_1s[[1]]$Vel[ind_event_1s]-mean(Vec_1s[[1]]$Vel[ind_event_1s]), col = "blue")
abline(h  = 0, col = "blue")
...
t.spec  <- spectrum(Vec_1s[[1]]$Vel[ind_event_1s], log="no", span=20, plot=FALSE)
...
require(dplR)
vec_unfil  <- Vec_1s[[1]]$P[ind_event_1s]
vec_fil    <- pass.filt(vec_unfil, W = c(0.1,0.5), type = "pass", method="Butterworth")
                        
png(file = paste("CHECK_FILTER",".png",sep=""), width=24, height=12, units="cm",res=300);
par(mfrow=c(1,1),cex.axis=0.85, oma = c(4,4,2,0.5),
    mar=c(1,1,1,1),lwd=1,mgp=c(3,0.5,0), cex = 0.75);


plot(dat_AQP_wave_lowPT[[1]]$Timestamp[ind_event_1s], vec_unfil, col = "blue", type = 'l')
lines(dat_AQP_wave_lowPT[[1]]$Timestamp[ind_event_1s], vec_fil, col = "red", lty = 1)

dev.off()


vec_unfil  <- Vec_1s[[1]]$Vel[ind_event_1s]
vec_fil    <- pass.filt(vec_unfil, W = c(0.1), type = "low", method="Butterworth")

png(file = paste("CHECK_FILTER_VEL",".png",sep=""), width=24, height=12, units="cm",res=300);
par(mfrow=c(1,1),cex.axis=0.85, oma = c(4,4,2,0.5),
    mar=c(1,1,1,1),lwd=1,mgp=c(3,0.5,0), cex = 0.75);


plot(dat_AQP_wave_lowPT[[1]]$Timestamp[ind_event_1s], Vec_1s[[1]]$P[ind_event_1s], col = "blue", type = 'l')

plot(dat_AQP_wave_lowPT[[1]]$Timestamp[ind_event_1s], vec_fil, col = "red", lty = 1)

dev.off()


png(file = paste("CHECK_FILTER_VEL_ALL",".png",sep=""), width=24, height=12, units="cm",res=300);
par(mfrow=c(2,1),cex.axis=0.85, oma = c(4,4,2,0.5),
    mar=c(1,1,1,1),lwd=1,mgp=c(3,0.5,0), cex = 0.75);
...
plot(dat_AQP_wave_lowPT[[1]]$Timestamp[ind_event_1s], vec_fil, col = "red", type = 'l', lty = 1)
