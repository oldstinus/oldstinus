...
setwd('C:/Users/meiredk/Desktop/Aquadopp_x3/11968_laag_1Hz')
...
###
setwd('P:/24_077-Golfw_IGG/3_Uitvoering/4_Analyse/1_testmeting/2_data/0_summary_ctu')
save(data2, file = "Rawdata_AQP.Rdata")

#load('Rawdata_laag.Rdata')
data <- data2
rm(data2)
...
AQP_wave_low_1m <- Velocity

setwd('P:/24_077-Golfw_IGG/3_Uitvoering/4_Analyse/1_testmeting/2_data/0_summary_ctu')
save(AQP_wave_low_1m, file = "AQP_wave_low_1mb.Rdata")
...
for (i in 1:len){
  ...
  #
  ind <- which(AQP_wave_low_1m$Timestamp > LW$Timestamp[i] & AQP_wave_low_1m$Timestamp < LW$Timestamp[i+1])
  for (j in 1:9){
    
    if (j < 5) out_1m_PT[[j]] <- AQP_wave_low_1m[[j]][ind]
    if (j > 4 & j < 7) out_1m_PT[[j]] <- AQP_wave_low_1m[[j]][ind,]
    if (j == 7) out_1m_PT[[j]] <- AQP_wave_low_1m[[j]][ind]
    if (j == 8) out_1m_PT[[j]] <- AQP_wave_low_1m[[j]]
    if (j == 9) out_1m_PT[[j]] <- AQP_wave_low_1m[[j]][ind]
    
  }
  names(out_1m_PT) <- names(AQP_wave_low_1m)
  out_1m_PTl[[i]] <- out_1m_PT
  
}

AQP_wave_lowPT_1m <- out_1m_PTl
setwd('P:/24_077-Golfw_IGG/3_Uitvoering/4_Analyse/1_testmeting/2_data/0_summary_ptide')
save(AQP_wave_lowPT_1m, file = "AQP_wave_lowPT_1mb.Rdata")
...
AQP_wave_low <- Velocity

setwd('P:/24_077-Golfw_IGG/3_Uitvoering/4_Analyse/1_testmeting/2_data/0_summary_ctu')
save(AQP_wave_low, file = 'AQP_wave_lowb.Rdata')
...
for (i in 1:len){
  ...
  #
  ind <- which(AQP_wave_low$Timestamp > LW$Timestamp[i] & AQP_wave_low$Timestamp < LW$Timestamp[i+1])
  for (j in 1:9){
    if (j > 1 & j < 5) out_PT[[j]] <- AQP_wave_low[[j]][ind]
    if (j > 4 & j < 7) out_PT[[j]] <- AQP_wave_low[[j]][ind,]
    if (j == 7) out_PT[[j]] <- AQP_wave_low[[j]][ind]
    if (j == 8) out_PT[[j]] <- AQP_wave_low[[j]]
    if (j == 9) out_PT[[j]] <- AQP_wave_low[[j]][ind]
    
  }
  
  names(out_PT) <- names(AQP_wave_low)
  out_PTl[[i]] <- out_PT
  
}

setwd('P:/24_077-Golfw_IGG/3_Uitvoering/4_Analyse/1_testmeting/2_data/0_summary_ptide')
AQP_wave_lowPT <- out_PTl
save(AQP_wave_lowPT, file = "AQP_wave_lowPTb.Rdata")
