...
from PIL import Image, ImageTk  # Voor het verwerken van afbeeldingen
...
def main():
    ...
    def select_logo():
        ...
        if logo_path:
            ...
            update_logo_display(logo_path)

    # Functie om logo in de title frame te tonen, 3x groter
    def update_logo_display(logo_path):
        try:
            img = Image.open(logo_path)
            # Bereken nieuwe grootte (3x)
            new_size = (int(img.width * 3), int(img.height * 3))
            img = img.resize(new_size, Image.ANTIALIAS)
            logo_img = ImageTk.PhotoImage(img)
            logo_display.config(image=logo_img)
            logo_display.image = logo_img  # Bewaar referentie
        except Exception as e:
            messagebox.showerror("Fout", f"Kan logo niet laden: {e}")
    ...
    def generate_plot():
        ...
        try:
            ...
            if logo_path:
                ...
                # Voeg afbeelding toe aan layout
    ...
    # Logo display in title frame
    logo_display = tk.Label(title_frame)
    logo_display.pack(side=tk.LEFT, padx=10)
