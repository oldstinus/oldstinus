...
<body>
  <div class="panel">
    ...
    <p>Kies een <b>punt</b> voor een stationaire absolute referentie.</p>
    ...
    <p>Klik op de kaart om het referentiepunt te kiezen.</p>
  ...
  <script>
    ...
    function updateStatus() {{
      if (selected.length === 0) {{
        statusEl.textContent = 'Nog niets geselecteerd.';
        return;
      }}
      if (mode === 'point') {{
        const p = selected[0];
      ...
      statusEl.textContent = 'Onbekende selectie.';
    }}
    ...
    for i, d in enumerate(datasets):
        ...
        for c in range(nc):
            ...
            for r in range(nr):
                ...
                surf_text[r, c] = (
                    ...
                    f"ensemble={ens}<br>"
                    f"{ts}<br>"
                    f"E={float(d.x[r, c]):.3f}<br>"
                    f"N={float(d.y[r, c]):.3f}<br>"
        ...
        for k in range(len(d.track_e)):
            ...
            track_text.append(
                ...
                f"ensemble={ens}<br>"
                f"{ts}<br>"
                f"E={float(d.track_e[k]):.3f}<br>"
                f"N={float(d.track_n[k]):.3f}<br>"
        ...
        if np.count_nonzero(valid_track) >= 2:
            ...
            compact_label = d.processing_mode in {"raai_projected", "raai_fallback"} or local_span <= 3.0
...
class App(tk.Tk):
    ...
    def _build(self) -> None:
        ...
        ttk.Label(opt, text="Absolute referentie KMZ/KML punt:").grid(row=7, column=0, sticky="w", pady=(8, 0))
