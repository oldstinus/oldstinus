...
def calculate_discharge(mat_filepath):
    """
    Voor de gegeven MAT-file wordt er een debiet (Q) berekend per transect.
    Hierbij wordt aangenomen dat:
      - De file de structuren "Summary", "System", "BottomTrack" en "WaterTrack" bevat.
      - 'Summary["Track"]' (NS x 2) de ADCP-posities bevat.
      - De horizontale snelheid wordt verkregen uit WaterTrack["Velocity"] (NC x 4 x NS)
        (waarbij de eerste twee componenten de u- en v-snelheden zijn).
      - Voor iedere ADCP-ping (index i) wordt:
           * De verticale profieldiepte bepaald via Cell_Start en Cell_Size,
           * Alleen cellen voor de bodem (BT_Depth) in aanmerking genomen,
           * De gemiddelde normale snelheid (t.o.v. de transectrichting) berekend,
           * De effectieve waterkolom (verticale dikte) en horizontale breedte geschat.
      - Het debiet per ping is dan: Q_i = (gemiddelde normale snelheid) * (verticale dikte) * (breedte)
      - Het totale debiet wordt als de som over alle pings genomen.
    ...
    # Verkrijg de track (ADCP-posities)
    track = np.array(summary["Track"])  # (NS, 2)
    NS = track.shape[0]
    # Bepaal de transectrichting (als er minstens 2 pings zijn)
    if NS < 2:
        t_dir = np.array([1, 0])
    else:
        d = track[-1] - track[0]
        ...
        if norm_d == 0:
            ...
            t_dir = d / norm_d
    # Normaalvector op transect (teken wordt onderaan nog gecorrigeerd
    # naar de conventie links->rechts positief).
    n_vec = np.array([t_dir[1], -t_dir[0]])

    # Bepaal de horizontale breedte per ping.
    widths = np.zeros(NS)
    if NS == 1:
        widths[0] = 1.0
    else:
        widths[0] = np.linalg.norm(track[1] - track[0]) / 2.0
        widths[-1] = np.linalg.norm(track[-1] - track[-2]) / 2.0
        for i in range(1, NS - 1):
            widths[i] = (np.linalg.norm(track[i] - track[i - 1]) +
                         np.linalg.norm(track[i + 1] - track[i])) / 2.0

    # Haal overige systeemdata op
    ...
    NC, comp, _ = velocity.shape
    if comp < 2:
        raise ValueError("Velocity array bevat minder dan 2 componenten.")
    u = velocity[:, 0, :]  # (NC, NS)
    v = velocity[:, 1, :]  # (NC, NS)

    total_discharge = 0.0
    # Loop over iedere ADCP-ping
    for i in range(NS):
        # Bepaal de verticale positie van de cellen
        z_raw = cstart[i] + (np.arange(NC) + 0.5) * csize[i]
        valid = z_raw < bt_depth[i]
        if not np.any(valid):
            continue
        # Bepaal de normale snelheden in de geldige cellen
        u_valid = u[valid, i]
        v_valid = v[valid, i]
        normal_vel = u_valid * n_vec[0] + v_valid * n_vec[1]
        finite_vel = np.isfinite(normal_vel)
        if not np.any(finite_vel):
            continue
        avg_normal = np.mean(normal_vel[finite_vel])
        # Bereken de verticale dikte (ofwel het verschil tussen de bovenste en onderste geldige cel)
        thickness = z_raw[valid][-1] - z_raw[valid][0]
        if not np.isfinite(thickness) or thickness == 0:
            thickness = csize[i] * np.sum(valid)
        if not np.isfinite(thickness) or thickness <= 0:
            continue
        if not np.isfinite(widths[i]):
            continue
        # Effectieve oppervlakte voor deze ping
        area = thickness * widths[i]
        if not np.isfinite(area):
            continue
        discharge_i = avg_normal * area
        if not np.isfinite(discharge_i):
            continue
        total_discharge += discharge_i

    # Conventie: als transect in X-richting links->rechts loopt,
    # dwing negatieve Q naar positief.
    left_to_right = False
    if track.ndim == 2 and track.shape[0] >= 2 and track.shape[1] >= 1:
        x0 = track[0, 0]
        x1 = track[-1, 0]
        if np.isfinite(x0) and np.isfinite(x1):
            left_to_right = x1 > x0
    if left_to_right and total_discharge < 0:
        total_discharge = -total_discharge

    # Bepaal de transecttijd (hier: het minimum van System["Time"])
    time_arr = np.array(system["Time"]).squeeze()
    transect_time = np.min(time_arr)

    return transect_time, total_discharge
...
class MatFilesTableGUI:
    ...
    def plot_time_series(self):
        ...
        ax.set_ylabel("Debiet (links->rechts positief) (m3/s)")
        ax.set_title("Tijdseries van totale debieten per transect")
        ax.grid(True)
        ...
        def update_annot(ind):
            ...
            annot.set_text(
                ...
                f"Debiet (links->rechts +): {discharges_dt[idx]:+.3f} m3/s"
            )
