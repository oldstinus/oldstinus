...
# -*- coding: utf-8 -*-
"""
BATHY GUI – 1D-lasso → Interpolatie/Extrapolatie → 3D-raster bewerken (zonder her-interpolatie)
- Éénmalige keuze VB_Depth of BT_Depth (per sessie).
- Positie: DGPS (UTM of Lat/Lon→UTM); zo niet: Summary.Track + UTM-anker.
...
_DEPTH_CHOICE = None  # 'vb' of 'bt' — éénmalig per sessie
...
def extract_depth(data):
    ...
    if _DEPTH_CHOICE is None:
        if vz is not None and bz is not None:
            ...
            _DEPTH_CHOICE = 'vb' if vz is not None else 'bt'

    Z = _as_array(vz if _DEPTH_CHOICE == 'vb' else bz).astype(float)
    # tijd
    t_depth = None
...
class BathyModel:
    def interpolate(self, X, Y, Z, grid_res=150, extrapolate=False):
        ...
        if _nearly_collinear(X, Y) or np.ptp(X)==0 or np.ptp(Y)==0:
            raise ValueError("XY (bijna) 1D – raster niet zinvol.")
        ...
        for method in ('cubic', 'linear'):
            try:
                ...
                continue
        if Zi is None:
            # laatste redmiddel – jitter + linear
        ...
        if extrapolate:
            # vul NaN via nearest vanaf de ruwe punten (eenmalig, geen “her-interpolatie” bij bewerken)
...
class Lasso2DSelector:
    ...
    def __init__(self, pts2d, title="2D selectie (Lasso)"):
        ...
        self.ax.set_title(title + " — Lasso tekenen; D/Enter=bevestig, U=reset, Q=annuleer")
        self.ax.set_xlabel("screen x (px)"); self.ax.set_ylabel("screen y (px)")
        self.ax.invert_yaxis()  # scherm-coördinaten: y naar beneden
...
class RasterEditor3D:
    ...
    Werkwijze:
      ...
      - Druk L → er opent een 2D scatter van de geprojecteerde grid-nodes (geen lijnen).
      - Lasso in 2D, D/Enter bevestigt → de gekozen grid-nodes worden op NaN gezet.
      - U = undo, Q = sluit.
    """
    ...
    def _draw(self, subtitle=None):
        ...
        title = "3D Rasterbewerker — L: 2D-lasso  D: delete  U: undo  Q: quit"
...
class BathyGUI:
    def __init__(self, root):
        root.title("Bathy GUI – 1D → 3D + Rasterbewerker (zonder her-interpolatie)")
    ...
    def edit_raster_3d(self):
        ...
        try:
            dil = simpledialog.askinteger("Selectie-dilatie", "Aantal dilatie-iteraties (0–3):",
