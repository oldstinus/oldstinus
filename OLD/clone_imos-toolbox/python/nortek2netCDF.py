...
def parse_file(filepath, update_status=None):
    """
    Parses the Nortek file and converts it to netCDF.
    If update_status is provided, it should be a function that takes a string to update the GUI.
    """
    checksum_errors = 0
    no_sync = 0
    sample_count = 0
    ...
    try:
            ...
            while data:
                ...
                if data == b'\xa5':  # sync
                    ...
                    if checksum & 0xffff != struct.unpack("<H", packet[-2:])[0]:
                        if update_status:
                            update_status(f"Check sum error at position {bad_ck_pos}")
                        else:
                            print("check sum error ", bad_ck_pos, checksum & 0xffff, (struct.unpack("<H", packet[-2:])[0]))
                        checksum_errors += 1
                        if checksum_errors > 10:
                            if update_status:
                                update_status("Too many errors, maybe not a nortek file")
                            else:
                        ...
                        try:
                            ...
                            for k in keys:
                                if k.endswith("..."):
                                    ...
                                    for i in range(0, number_samples):
                                        keys_out.append(f"{kn}[{i}]")
                                else:
                                    keys_out.append(k)
                            # print(keys_out)

                            if 'Vector With IMU' == packet_decoder[id]['name']:
                                AHRS_id = struct.unpack("<B", packet[1:2])[0]
                            ...
                            if 'serial' in d:
                                ...
                                if update_status:
                                    update_status(f"Instrument serial number: {instrument_serialnumber}")
                            ...
                            if 'head_serial' in d:
                                ...
                                if update_status:
                                    update_status(f"Instrument head serial number: {instrument_head_serialnumber}")
                            ...
                            if 'system' in d:
                                ...
                                for sd in d['system']:
                                    print(sd)
                                if update_status:
                                    update_status(f"System Data: {d['system']}")
                            ...
                            if 'Vector With IMU' == packet_decoder[id]['name']:
                                ...
                                if len(vector_imu_data) % 1000 == 0:
                                    print(f"samples read {len(vector_imu_data)}, ts: {ts}, dt: {dt}, delta: {(dt - ts).total_seconds()}, total: {(dt-first_time).total_seconds()}")
                                    if update_status:
                                        update_status(f"samples read {len(vector_imu_data)}")
                            ...
                            if update_status:
                                update_status(f"Packet decode not found for ID {id}")
                else:
                    no_sync += 1
                    # if no_sync > 100:
        ...
        if update_status:
            update_status("Bestand niet gevonden.")
        else:
            print("Bestand niet gevonden.")
        return None
    except Exception as e:
        if update_status:
            update_status(f"Fout tijdens het parsen: {e}")
        else:
            print(f"Fout tijdens het parsen: {e}")
        return None

    if update_status:
        update_status('Parsing voltooid.')
    ...
    if number_samples_read == 0:
        if update_status:
            update_status("Geen samples gevonden, waarschijnlijk geen Nortek Aquadopp of Vector bestand.")
        else:
    ...
    if update_status:
        update_status(f"Creëren van netCDF bestand: {outputName}")
    ...
    if aquadopp:
        ...
        if vel_raw_mag:
            data_array = np.full((15, number_samples_read), np.nan)
        else:
            data_array = np.full((11, number_samples_read), np.nan)
    ...
    if aquapro:
        ...
        # create an array to store data in (creates another memory copy)
        data_array = np.full((8, number_samples_read), np.nan)
        profile_array = np.full((number_beams, number_samples_read, number_bins), np.nan)
    ...
    if vector:
        ...
        # create an array to store data in (creates another memory copy)
        data_array = np.full((3, number_samples_read), np.nan)
        vector_array = np.full((3, number_samples_read, 3), np.nan)
        mat_array = np.full((1, number_samples_read, 9), np.nan)

        for i in range(number_samples_read):
    ...
    if coord_system is not None and coord_system < len(coord_systems):
        ncOut.coord_system = coord_systems[coord_system]
    else:
        ncOut.coord_system = 'Unknown'
    ...
    ncOut.setncattr("date_created", datetime.datetime.now(timezone.utc).strftime(ncTimeFormat))
    ncOut.setncattr("history", datetime.datetime.now(timezone.utc).strftime("%Y-%m-%d") + " created from file " + filepath)

    ncOut.close()

    if update_status:
        update_status(f"Conversie voltooid: {outputName}")

    return outputName


def select_file_and_convert(update_status_callback):
    """
    Opens a file dialog for the user to select a file and starts the conversion.
    """
    filepath = filedialog.askopenfilename(title="Selecteer Nortek data bestand", filetypes=[("All files", "*.*")])
    if not filepath:
        return  # User cancelled

    # Start parsing in a separate thread to keep the GUI responsive
    threading.Thread(target=parse_and_notify, args=(filepath, update_status_callback)).start()


def parse_and_notify(filepath, update_status_callback):
    """
    Parses the file and notifies the user upon completion.
    """
    update_status_callback("Start conversie...")
    output = parse_file(filepath, update_status_callback)
    if output:
        messagebox.showinfo("Succes", f"Conversie voltooid:\n{output}")
    else:
        messagebox.showerror("Fout", "Conversie mislukt. Controleer de log voor details.")


def main():
    # Initialize the main window
    root = tk.Tk()
    root.title("Nortek naar netCDF Converter")
    ...
    # Add a label
    label = tk.Label(root, text="Selecteer een Nortek data bestand om te converteren naar netCDF.", wraplength=400, justify="center")
    label.pack(pady=20)

    # Add a status label
    status_label = tk.Label(root, text="Status: Klaar", anchor="w")
    ...
    select_button = tk.Button(root, text="Selecteer Bestand", command=lambda: select_file_and_convert(update_status), width=20, height=2)
