...
OUTPUT_HTML = ROOT / "docs" / "oldstinus_overzicht.html"
OUTPUT_JSON = ROOT / "docs" / "oldstinus_overzicht.json"
LAUNCHERS_DIR = ROOT / "docs" / "launchers"
...
def extract_doc_hint(lines: list[str]) -> str:
    ...
    if docstring_match:
        extracted = [clean_sentence(line) for line in docstring_match.group(1).strip().splitlines() if clean_sentence(line)]
        if extracted:
            return extracted[0]
...
def classify_file(path: Path) -> ScriptInfo:
    ...
    top_level = rel_parts[0] if len(rel_parts) > 1 else ROOT.name
...
def rel_href(from_dir: Path, to_path: Path) -> str:
    rel_path = root_relative_path(to_path)
    if rel_path is None:
        raise ValueError(f"Pad ligt buiten ROOT: {to_path}")
    rel = rel_path.as_posix()
    rel_from_docs = Path("..") / Path(rel)
    return quote(rel_from_docs.as_posix(), safe="/#():")
...
def render_card(item: ScriptInfo) -> str:
    file_link = rel_href(OUTPUT_HTML.parent, ROOT / item.rel_path)
    launcher_link = (ROOT / "docs" / item.launcher_rel_path).resolve().as_uri()
    editor_link = vscode_href(ROOT / item.rel_path)
    ...
    project_label = item.project_dir if item.project_dir not in ("", ".") else ROOT.name
...
def generate_launcher(item: ScriptInfo) -> None:
    ...
    script_path = ROOT / item.rel_path
...
def build_html(items: list[ScriptInfo]) -> str:
    ...
    operation_counts = Counter(op for item in items for op in item.operations)
    project_counts = Counter(item.project for item in items)
    priority_items = sorted(
        [item for item in items if item.is_priority],
    ...
    quick_links = "".join(
        f'<a href="#{slugify(f"category-{category}")}">{html.escape(category)}</a>'
        for category in sorted(counts)
    )
...
<body>
  <div class="shell">
    ...
    <section class="note">
      ...
      De pagina wordt opnieuw opgebouwd door <code>docs/generate_oldstinus_catalog.py</code>.
    </section>
  </div>
