...
- Alleen lijnen (geen markers of balken)
- Hover per lijn (alleen de lijn onder de muis)
- Lijst met checkboxen om reeksen aan/uit te zetten (naast de legenda)
- Y-as min/max instellen (Toepassen) + Auto-knop
- X-as range-selector (1u, 6u, 12u, 1d, 3d, alles) en range-slider
...
def parse_multistation_csv(path: str | Path) -> pd.DataFrame:
    ...
    for ln in lines:
        ...
        if ln.startswith("#"):
            # Kolomkop data -> vanaf nu datarijen
...
# =============== HTML TEMPLATE + WRITER (standalone, offline) ===============
...
import html
import plotly.graph_objects as go
import plotly.io as pio  # <-- belangrijk

def write_interactive_html(fig: go.Figure, out_path: Path) -> None:
    """
    Schrijf standalone HTML met:
      - Plotly inline (offline)
      - Reeksvinkjes (Alle/Geen)
      - Y-as min/max Apply + Auto
    """
    # 1) Haal de Plotly JS-lib inline uit pio.to_html(...)
    inline = pio.to_html(fig, include_plotlyjs="inline", full_html=False)
    start = inline.find("<script>")
    end   = inline.find("</script>", start)
    lib_js = inline[start+len("<script>"):end] if start != -1 and end != -1 else ""

    # 2) Serialize de figure veilig met Plotly zelf (vermijdt ndarray-issues)
    fig_json_str = pio.to_json(fig, pretty=False)  # <-- JSON string, klaar voor embedden
    ...
    html_txt = TEMPLATE.format(
        ...
        plotly_js=lib_js,
        fig_json=fig_json_str           # <-- direct de string, NIET json.dumps(...)
    )
