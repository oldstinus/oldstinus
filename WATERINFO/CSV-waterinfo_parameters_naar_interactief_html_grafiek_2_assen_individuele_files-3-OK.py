...
# -*- coding: utf-8 -*-
"""
CSV_waterinfo_multi_files_to_html_dual_axis_NO_LEGEND.py

Wijzigingen:
- GEEN Plotly-legende op de grafiek (showlegend=False).
- Reeksen-bediening uitsluitend in de kantlijn (checkboxen).
- Hover/muisover-informatie blijft actief en volledig.
- Trace-naam = "[Param (unit)] Station — Bestandsnaam" (instelbaar via name_mode).
- Optioneel één parameter op Y2 (rechter Y-as).

Compatibel met Waterinfo CSV's met multi-header + datablok.
"""
...
def load_many_csv(paths: list[Path]) -> pd.DataFrame:
    ...
    if not frames:
        return pd.DataFrame(columns=[
            "Timestamp", "Value", "station_name", "station_no", "parameter", "unit", "source_file"
        ])
...
def choose_secondary_parameter(root: tk.Tk, params: list[str]) -> str | None:
    ...
    tk.Label(win, text="Plaats één parameter op de rechter Y-as.\n"
...
def build_plot(df: pd.DataFrame,
               name_mode: str = "station_file",
               secondary_param: str | None = None) -> go.Figure:
    """
    name_mode:
      'station'      → trace = "[Param (unit)] Station"
      'file'         → trace = "[Param (unit)] Bestandsnaam"
      'station_file' → trace = "[Param (unit)] Station — Bestandsnaam" (default)
    secondary_param:
      None → alles op Y1
    ...
    units_map = (df.dropna(subset=["parameter"])
    ...
    # Groepering per trace
    if name_mode == "file":
        group_cols = ["source_file", "parameter", "unit"]
    elif name_mode == "station":
        group_cols = ["station_name", "parameter", "unit"]
    else:
    ...
    for keys, g in df.groupby(group_cols, dropna=False):
        ...
        station_label = str(g["station_name"].dropna().iloc[0]) if "station_name" in g and not g["station_name"].dropna().empty else "Onbekend station"
        file_label    = str(g["source_file"].dropna().iloc[0])  if "source_file"  in g and not g["source_file"].dropna().empty  else ""
        p             = str(g["parameter"].dropna().iloc[0])    if "parameter"    in g and not g["parameter"].dropna().empty    else ""
        u             = str(g["unit"].dropna().iloc[0])         if "unit"         in g and not g["unit"].dropna().empty         else ""

        # Trace-naam met parameter
        param_piece = f"[{p}" + (f" ({u})" if u else "") + "]"
        if name_mode == "file":
            base = file_label or "Bestand"
        elif name_mode == "station":
            base = station_label
        else:
            base = f"{station_label}" + (f" — {file_label}" if file_label else "")
        trace_name = f"{param_piece} {base}"

        # Hovertemplate (muisover)
        hover = (
            "<b>Trace:</b> " + trace_name + "<br>" +
            "<b>Tijd:</b> %{x}<br>" +
            "<b>Waarde:</b> %{y}<br>" +
            (f"<b>Parameter:</b> {p}" + (f" ({u})" if u else "") + "<br>" if p else "") +
            (f"<b>Station:</b> {station_label}<br>" if station_label else "") +
            (f"<b>Bestand:</b> {file_label}<br>" if file_label else "") +
            "<extra></extra>"
        )
    ...
    # Optionele dropdown (Alle/Geen/Alleen deze) blijft handig en stoort de eis niet
    n = len(list(fig.data))
    if n > 0:
        buttons = [
...
# ---------- HTML template met Y1 & Y2 bediening en alleen kantlijn-reeksen ----------
...
def main() -> None:
    ...
    # Naamgevingsmodus: 'station', 'file' of 'station_file'
    name_mode = "station_file"

    try:
        fig = build_plot(df, name_mode=name_mode, secondary_param=secondary_param)
    except Exception as e:
    ...
    y2_label = None
    if secondary_param:
        y2_label = fig.layout.yaxis2.title.text if hasattr(fig.layout, "yaxis2") else None
