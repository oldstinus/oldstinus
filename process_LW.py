...
"""
#******************************************************************************
# This program computes the wave characteristics of the long wave surface 
...
def calculate_Sf_params(S,f,fmin,fmax,delta_f):  
    ...
    index_f = [i[0] for i in np.argwhere((f>=fmin) & (f<=fmax))] 
...
datafile_eta = os.path.join(basedir,'druksensoren','eta',location+'_eta_LW_'+'%i' %fs +'Hz_'+st.strftime('%Y_%m_%d')+'_'+te.strftime('%Y_%m_%d')+output_suffix+ '.dat')
...
M = 512*fs                                #segment length for spectral averaging in short wave analysis (Table 3-2 IMDC report RA15039) 
p = 2                                   #number of segments (Table 3-2 IMDC report RA15039) 
...
#    fpcos[i]=f[Scos.argmax()]
    Hm0[i],Tm01[i],Tm02[i],Tm_10[i] = calculate_Sf_params(S,f,0.00833,0.05,delta_f)
#    Hm0cos[i],Tm01cos[i],Tm02cos[i],Tm_10cos[i] = calculate_Sf_params(Scos,f,0.05,0.43,delta_f)
    ...
    if st_Sf_out <= time <= et_Sf_out:
        ...
        data_Sf.to_csv(os.path.join(Spectrum_folder,'LW_Sf_'+location+'_'+datetime.strftime(time,'%d-%m-%Y %H-%M')+'.txt'),sep='\t',header=True,index=None)
...
spectrum_out.to_csv(os.path.join(Spectrum_folder,'LW_Hm0_Tm_10_'+location+'_15min.txt'),header=True,sep='\t',float_format = '%.3e')
