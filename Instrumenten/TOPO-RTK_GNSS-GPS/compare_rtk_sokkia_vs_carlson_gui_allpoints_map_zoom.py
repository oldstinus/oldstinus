...
def prepare_for_map(df: pd.DataFrame, crs_in: str, swap_en: bool, shift_s: float, kind: str) -> pd.DataFrame:
    """
    Voeg velden toe voor kaart:
      - t_shift (UTC)
      - E_L72, N_L72
      - lat, lon (WGS84)
    """
    ...
    out = df.copy()
    out["t_shift"] = out["time_utc"] + pd.to_timedelta(float(shift_s), unit="s")

    if swap_en:
        out[["E", "N"]] = out[["N", "E"]].values

    out["E"] = pd.to_numeric(out["E"], errors="coerce")
    out["N"] = pd.to_numeric(out["N"], errors="coerce")
    out["H"] = pd.to_numeric(out["H"], errors="coerce") if "H" in out.columns else np.nan
    out = out.dropna(subset=["E", "N", "time_utc"]).copy()
    out = out.sort_values("t_shift").reset_index(drop=True)

    E_l72, N_l72 = to_lambert72(out["E"].to_numpy(float), out["N"].to_numpy(float), crs_in)
    out["E_L72"], out["N_L72"] = E_l72, N_l72

    lat, lon = lambert72_to_wgs84(out["E_L72"].to_numpy(float), out["N_L72"].to_numpy(float))
...
def make_combined_map(
    sokkia_all: pd.DataFrame,
    carlson_all: pd.DataFrame,
    ...
    """
    Kaart met:
      - ALLE Sokkia punten (blauw) + track polyline
      - ALLE Carlson punten (rood) + track polyline
      - Match-lijnen (groen) (optioneel)
    Tooltip + popup bevatten Lambert72 + WGS84 + hoogte + tijd (orig + shifted).
    """
    ...
    lat_all = []
    lon_all = []
    for df in (sokkia_all, carlson_all):
        if df is not None and not df.empty and "lat" in df.columns and "lon" in df.columns:
            lat_all.append(df["lat"].median())
            lon_all.append(df["lon"].median())
    if lat_all and lon_all:
        center = [float(np.nanmedian(lat_all)), float(np.nanmedian(lon_all))]
        zoom = 17
    else:
        center = [50.85, 4.35]
        zoom = 8

    # Leaflet map: allow deeper zoom + fullscreen + scrollwheel
    mp = folium.Map(
        location=center,
        zoom_start=zoom,
        tiles=None,
    ...
    mp.options["scrollWheelZoom"] = True
    mp.options["doubleClickZoom"] = True
    mp.options["zoomControl"] = True

    # Base layers (met hogere max_zoom dan default)
    folium.TileLayer(
        tiles="OpenStreetMap",
        name="OpenStreetMap",
        control=True,
    ...
    # Plugins
    plugins.Fullscreen(position="topleft").add_to(mp)
    ...
    fg_s = folium.FeatureGroup(name="Sokkia (alle punten)", show=True)
    fg_c = folium.FeatureGroup(name="Carlson (alle punten)", show=True)
    fg_m = folium.FeatureGroup(name="Matches (lijnen)", show=True)

    # --- Sokkia points ---
    if sokkia_all is not None and not sokkia_all.empty:
        pts = sokkia_all[["lat", "lon"]].astype(float).values.tolist()
        if len(pts) >= 2:
            folium.PolyLine(pts, weight=4, opacity=0.8, color="blue").add_to(fg_s)

        ds = sokkia_all.iloc[::max(1, int(every_s))].copy()
        for idx, r in ds.iterrows():
            lat, lon = float(r["lat"]), float(r["lon"])
            E72, N72 = float(r["E_L72"]), float(r["N_L72"])
            H = r.get("H", np.nan)
            t0 = r.get("time_utc", "")
            ts = r.get("t_shift", "")
            sol = r.get("TypeSolution", "")
            station = r.get("Station", "")

            tip = f"SOKKIA | t={ts} | E72={E72:.3f} N72={N72:.3f} | lat={lat:.8f} lon={lon:.8f} | H={H}"
            pop = folium.Popup(
                f"<b>SOKKIA</b><br>"
                f"<b>Time (raw)</b>: {t0}<br>"
                f"<b>Time (shift)</b>: {ts}<br>"
                f"<b>Lambert72</b>: E={E72:.3f} m, N={N72:.3f} m<br>"
                f"<b>WGS84</b>: lat={lat:.8f}, lon={lon:.8f}<br>"
                f"<b>H</b>: {H}<br>"
                f"<b>TypeSolution</b>: {sol}<br>"
                f"<b>Station</b>: {station}",
                max_width=520,
            )
            folium.CircleMarker([lat, lon], radius=3, color="blue", fill=True, fill_opacity=0.9,
                                tooltip=tip, popup=pop).add_to(fg_s)

        # start/end
        r0 = sokkia_all.iloc[0]
        r1 = sokkia_all.iloc[-1]
        for label, rr in [("SOKKIA START", r0), ("SOKKIA END", r1)]:
            lat, lon = float(rr["lat"]), float(rr["lon"])
            E72, N72 = float(rr["E_L72"]), float(rr["N_L72"])
            tip = f"{label} | E72={E72:.3f} N72={N72:.3f} | lat={lat:.8f} lon={lon:.8f}"
            pop = folium.Popup(
                f"<b>{label}</b><br>"
                f"<b>Lambert72</b>: E={E72:.3f} m, N={N72:.3f} m<br>"
                f"<b>WGS84</b>: lat={lat:.8f}, lon={lon:.8f}",
                max_width=420
            )
            folium.Marker([lat, lon], tooltip=tip, popup=pop).add_to(fg_s)

    # --- Carlson points ---
    if carlson_all is not None and not carlson_all.empty:
        pts = carlson_all[["lat", "lon"]].astype(float).values.tolist()
        if len(pts) >= 2:
            folium.PolyLine(pts, weight=4, opacity=0.8, color="red").add_to(fg_c)

        ds = carlson_all.iloc[::max(1, int(every_c))].copy()
        for idx, r in ds.iterrows():
            lat, lon = float(r["lat"]), float(r["lon"])
            E72, N72 = float(r["E_L72"]), float(r["N_L72"])
            H = r.get("H", np.nan)
            ...
            tip = f"CARLSON | t={ts} | E72={E72:.3f} N72={N72:.3f} | lat={lat:.8f} lon={lon:.8f} | H={H}"
            pop = folium.Popup(
                f"<b>CARLSON</b><br>"
                f"<b>Time (raw)</b>: {t0}<br>"
                f"<b>Time (shift)</b>: {ts}<br>"
            ...
            folium.CircleMarker([lat, lon], radius=3, color="red", fill=True, fill_opacity=0.9,
                                tooltip=tip, popup=pop).add_to(fg_c)

        r0 = carlson_all.iloc[0]
        r1 = carlson_all.iloc[-1]
        for label, rr in [("CARLSON START", r0), ("CARLSON END", r1)]:
            lat, lon = float(rr["lat"]), float(rr["lon"])
            E72, N72 = float(rr["E_L72"]), float(rr["N_L72"])
            tip = f"{label} | E72={E72:.3f} N72={N72:.3f} | lat={lat:.8f} lon={lon:.8f}"
            pop = folium.Popup(
            ...
            folium.Marker([lat, lon], tooltip=tip, popup=pop).add_to(fg_c)

    # --- Match lines ---
    if matches is not None and not matches.empty:
        ds = matches.iloc[::max(1, int(every_m))].copy()
        for i, r in ds.iterrows():
            tip_line = f"dXY={r['dXY_m']:.3f} m | dE={r['dE_m']:.3f} m | dN={r['dN_m']:.3f} m | dH={r.get('dH_m', np.nan)} m | dt={r.get('dt_s', np.nan)} s"
            folium.PolyLine(
                locations=[[r["lat_sokkia"], r["lon_sokkia"]], [r["lat_carlson"], r["lon_carlson"]]],
                weight=2, opacity=0.8, tooltip=tip_line, color="green"
            ).add_to(fg_m)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.var_map_every_s = tk.IntVar(value=5)
        self.var_map_every_c = tk.IntVar(value=5)
        self.var_map_every_m = tk.IntVar(value=1)
        ...
        self._s_all: Optional[pd.DataFrame] = None
        self._c_all: Optional[pd.DataFrame] = None
    ...
    def _build(self):
        ...
        ttk.Label(opt, text="Kaart thinning (n): Sokkia").grid(row=3, column=2, sticky="w", padx=(16,0), pady=(8,0))
        ttk.Entry(opt, textvariable=self.var_map_every_s, width=8).grid(row=3, column=3, sticky="w", padx=(6,0), pady=(8,0))
        ttk.Label(opt, text="Carlson").grid(row=4, column=2, sticky="w", padx=(16,0), pady=(8,0))
        ttk.Entry(opt, textvariable=self.var_map_every_c, width=8).grid(row=4, column=3, sticky="w", padx=(6,0), pady=(8,0))
        ttk.Label(opt, text="Matches").grid(row=5, column=2, sticky="w", padx=(16,0), pady=(8,0))
        ttk.Entry(opt, textvariable=self.var_map_every_m, width=8).grid(row=5, column=3, sticky="w", padx=(6,0), pady=(8,0))
    ...
    def _run(self):
        try:
            ...
            # Prepare full datasets for map (alle punten)
            s_all = prepare_for_map(self.df_s, cfg.sokkia_crs, cfg.swap_sokkia_en, cfg.sokkia_shift_s, kind="sokkia")
            c_all = prepare_for_map(self.df_c, cfg.carlson_crs, cfg.swap_carlson_en, cfg.carlson_shift_s, kind="carlson")
            self._s_all = s_all
            self._c_all = c_all
            make_combined_map(s_all, c_all, m, out_html=out_map,
                             every_s=int(self.var_map_every_s.get()),
                             every_c=int(self.var_map_every_c.get()),
                             every_m=int(self.var_map_every_m.get()))
    ...
    def _make_map_only(self):
        try:
            ...
            out_map = str(Path(outb).with_suffix("")) + "_map.html"
            # Gebruik laatst berekende full datasets als beschikbaar
            s_all = getattr(self, "_s_all", None)
            c_all = getattr(self, "_c_all", None)
            if s_all is None or c_all is None:
                # fallback: opnieuw voorbereiden uit ruwe data
                cfg = MatchConfig(
                    sokkia_shift_s=float(self.var_shift_s.get()),
                    carlson_shift_s=float(self.var_shift_c.get()),
                    tol_s=float(self.var_tol.get()),
                ...
                s_all = prepare_for_map(self.df_s, cfg.sokkia_crs, cfg.swap_sokkia_en, cfg.sokkia_shift_s, kind="sokkia")
                c_all = prepare_for_map(self.df_c, cfg.carlson_crs, cfg.swap_carlson_en, cfg.carlson_shift_s, kind="carlson")
            make_combined_map(s_all, c_all, self.df_match, out_html=out_map,
                             every_s=int(self.var_map_every_s.get()),
                             every_c=int(self.var_map_every_c.get()),
                             every_m=int(self.var_map_every_m.get()))
            self._log(f"Saved: {out_map}")
