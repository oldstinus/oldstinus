...
3) GUI-gestuurde herwerking:
   ...
   - celbereik (Cel START/END) instellen  
...
def detect_cutoff_from_snr(distances, snr_profile, z=3.0):
    ...
    dp = np.diff(ps)  # per bin (typisch 4 cm per cel)
...
def plot_snr_heatmaps(self, time, per_beam, cutoff_by_beam):
    ...
    for beam in sorted(per_beam.keys()):
        ...
        # Set extent to cover full vertical range of cells (not limited by cutoff)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.cell_start = tk.IntVar(value=1)
        self.cell_end = tk.IntVar(value=999)
    ...
    def _build(self):
        ...
        # Selectie voor gemiddelde snelheid
        ...
        row2 = ttk.Frame(r4); row2.pack(fill="x", pady=2)
        ttk.Label(row2, text="Cel start:").pack(side="left", padx=pad)
        ttk.Entry(row2, textvariable=self.cell_start, width=10).pack(side="left")
        ttk.Label(row2, text="Cel eind:").pack(side="left", padx=pad)
        ttk.Entry(row2, textvariable=self.cell_end, width=10).pack(side="left")
    ...
    def on_load(self):
        ...
        # Standaard celbereik = alle beschikbare cellen
        self.cell_start.set(min(self.cells))
        self.cell_end.set(max(self.cells))

        # Auto cutoff per beam op basis van SNR
        ...
        if self.auto_cutoff.get():
            ...
            for b in (1,2,3,4):
                ...
                if M is None:
                    ...
                    continue
                # neem mediane SNR-profiel over de tijd:
                prof = np.array([
                    np.nanmedian(M[:, i]) if np.any(np.isfinite(M[:, i])) else np.nan 
    ...
    def on_recalc(self):
        ...
        cell_start = int(self.cell_start.get()); cell_end = int(self.cell_end.get())

        # Selecteer cellen die binnen zowel de ingestelde afstand/hoogte-range (hmin–hmax)
        # als binnen het opgegeven celbereik ('Cel start'–'Cel eind') vallen.
        # Gebruik voor elke cel de verticale locatie uit de data: 'Center' indien beschikbaar (beams 1&2),
        # anders 'Skew' (beams 3&4). Deze locaties (m) geven de afstand van het instrument 
        # tot het midden van de cel (aanname: SonTek IQ beams 1&2 in centraal vlak, 3&4 in scheef vlak).
        # Cellen buiten de selectie worden genegeerd.
        selected_cells = []
        for cell in self.cells:
            dc = f"Cell{cell} Location (Center) (m)"
            ds = f"Cell{cell} Location (Skew) (m)"
            d = np.nan
            if dc in df.columns:
                d = pd.to_numeric(df[dc].iloc[0], errors="coerce")
            elif ds in df.columns:
                d = pd.to_numeric(df[ds].iloc[0], errors="coerce")
            if np.isfinite(d) and (d >= hmin) and (d <= hmax) and (cell >= cell_start) and (cell <= cell_end):
                selected_cells.append(cell)

        if not selected_cells:
            messagebox.showerror("Fout", "Geen cellen binnen de gekozen afstand/hoogte-range en cel-range.")
            return

        def beam_bin_matrix(beam):
            # Bouw matrix (tijd x cellen) voor deze beam.
            # Voor elke geselecteerde cel bepalen we de verticale afstand (Center/Skew kolom) en verzamelen de snelheidswaarden.
            # Als er een cutoff-afstand is ingesteld voor deze beam (max. bruikbare afstand),
            # markeren we alle waarden van cellen die dieper zijn dan de cutoff als NaN.
            # (Aanname: data voorbij de cutoff is onbetrouwbaar en wordt genegeerd.)
            cols = []
            dists = []
            for cell in selected_cells:
                ...
                loc = f"Cell{cell} Location (Center) (m)" if beam in (1,2) else f"Cell{cell} Location (Skew) (m)"
                d = pd.to_numeric(df[loc].iloc[0], errors="coerce") if loc in df.columns else np.nan
        ...
        # Skip beams zonder geldige data of cutoff (niet gebruiken in berekening)
        if self.beam_on[1].get():
            if B1 is None:
                self._log("Beam 1 niet gebruikt (geen data binnen geselecteerde cellen).")
            elif not np.isfinite(float(self.max_dist[1].get())):
                self._log("Beam 1 niet gebruikt (geen geldige cutoff).")
                b1 = None
        if self.beam_on[2].get():
            if B2 is None:
                self._log("Beam 2 niet gebruikt (geen data binnen geselecteerde cellen).")
            elif not np.isfinite(float(self.max_dist[2].get())):
                self._log("Beam 2 niet gebruikt (geen geldige cutoff).")
                b2 = None
        if self.beam_on[3].get():
            if B3 is None:
                self._log("Beam 3 niet gebruikt (geen data binnen geselecteerde cellen).")
            elif not np.isfinite(float(self.max_dist[3].get())):
                self._log("Beam 3 niet gebruikt (geen geldige cutoff).")
                b3 = None
        if self.beam_on[4].get():
            if B4 is None:
                self._log("Beam 4 niet gebruikt (geen data binnen geselecteerde cellen).")
            elif not np.isfinite(float(self.max_dist[4].get())):
                self._log("Beam 4 niet gebruikt (geen geldige cutoff).")
                b4 = None

        # Bereken horizontale componenten (Janus projectie met θ):
        # Beam 1 & 2 -> u-component, Beam 3 & 4 -> v-component.
        # Verwachte formule: u = (b1 - b2) / (2 * sin θ), v = (b4 - b3) / (2 * sin θ).
        # Hier berekenen we u en v zonder de sin(θ) factor; de hoekfactor wordt toegepast in specifieke modes.
        # (Aanname: SonTek IQ heeft 4 beams onder een hoek θ=25° van verticaal.)
        ...
        # Snelheid berekenen volgens gekozen modus:
        # - "Resultant": combineer u en v tot resulterende horizontale snelheid (vector grootte).
        # - "Beam 1+2": bereken horizontale snelheid uit verschil van beam 1 en 2 (u-component, met sin θ factor).
        # - "Beam N": gebruik de gemiddelde snelheid van beam N (ruwe beamsnelheid, geen hoekcorrectie).
        if mode == "Resultant":
    ...
    def on_set_cutoff_manual(self):
        ...
        entries = {}
        for b in (1,2,3,4):
            row = ttk.Frame(dialog); row.pack(fill="x", pady=5)
