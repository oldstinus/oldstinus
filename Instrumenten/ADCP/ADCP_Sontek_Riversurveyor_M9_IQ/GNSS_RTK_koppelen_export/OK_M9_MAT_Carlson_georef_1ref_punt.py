...
Extra (v5):
- Bij geslaagde georeferering wordt automatisch een interactieve kaart (Folium HTML) gemaakt
  en geopend in de browser (optioneel via checkbox).
- Kaart toont:
  - georefererde M9 track (polyline)
  - (downsampled) meetpunten met popup: tijd + Lambert72 (E,N) + WGS84 (lat,lon)
  - geselecteerde ankerpositie als marker
...
def make_map_html(
    ...
    # anchor marker
    aE = float(anchor_row["E"])
    aN = float(anchor_row["N"])
    a_lon, a_lat = tr.transform(aE, aN)
    a_popup = (
        f"ANKER<br>"
        f"id={int(anchor_row['anchor_id'])} idx={anchor_row.get('idx','')}<br>"
        f"{str(anchor_row.get('time_utc',''))}<br>"
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        row += 1
        ttk.Entry(frm, textvariable=self.var_rtk, width=116).grid(row=row, column=0, sticky="we", padx=(0, 8))
        ...
        try:
            ...
            if bool(self.var_make_map.get()):
                if not _MAP_OK:
                    ...
                    out_html = str(base) + "_map.html"
                    anchor_row = self._rtk_df.loc[self._rtk_df["anchor_id"] == int(anchor_id)].iloc[0].copy()
