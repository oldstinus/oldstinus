...
def _reverse_profile_data(data: Track3DData) -> Track3DData:
    return replace(
        ...
        bed_depth=np.asarray(data.bed_depth, dtype=float)[::-1].copy(),
...
def build_parallel_figure(
    ...
    for i, d in enumerate(datasets):
        ...
        bed_depth = np.asarray(d_plot.bed_depth, dtype=float).reshape(-1)
        if bed_depth.size == tx.size:
            valid_bed = np.isfinite(tx) & np.isfinite(ty) & np.isfinite(bed_depth)
        else:
            valid_bed = np.zeros(tx.shape, dtype=bool)
        if np.count_nonzero(valid_bed) >= 2:
            fig.add_trace(
                go.Scatter3d(
                    x=np.where(valid_bed, tx, np.nan),
                    y=np.where(valid_bed, ty, np.nan),
                    z=np.where(valid_bed, -bed_depth, np.nan),
                    mode="lines",
                    line=dict(color="rgba(90,60,20,0.95)", width=5),
                    name=f"{d.path.stem} bodem",
                    legendgroup=grp,
                    showlegend=False,
                    connectgaps=False,
                    meta=dict(layer="bottom"),
                    customdata=np.where(valid_bed, bed_depth, np.nan),
                    hovertemplate=(
                        f"{d.path.name}<br>"
                        "Bodemdiepte=%{customdata:.3f} m<br>"
                        f"Profiel omgedraaid={'ja' if was_reversed else 'nee'}<br>"
                        f"Delta t={_fmt_delta(delta)}<br>"
                        f"Offset (L->R +)={offset_m:.2f} m<extra></extra>"
                    ),
