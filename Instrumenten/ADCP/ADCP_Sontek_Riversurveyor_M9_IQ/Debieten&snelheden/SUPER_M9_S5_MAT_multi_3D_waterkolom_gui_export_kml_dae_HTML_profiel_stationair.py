...
class MatPreviewSummary:
    ...
    ensembles: int
    start_edge_label: str = "n/a"
    end_edge_label: str = "n/a"
    profile_reversed: bool = False
    note: str = ""
...
def _mat_start_edge_label(mat_path: str | Path) -> str | None:
    try:
        mat = sio.loadmat(str(Path(mat_path)), squeeze_me=True, struct_as_record=False)
        setup = mat.get("Setup", None)
        if setup is None or not hasattr(setup, "startEdge"):
            return None
        val = np.asarray(getattr(setup, "startEdge"), dtype=float).reshape(-1)
        if val.size == 0 or not np.isfinite(val[0]):
            return None
        return "right" if float(val[0]) > 0.1 else "left"
    except Exception:
        return None


def _opposite_edge_label(edge_label: str | None) -> str | None:
    if edge_label == "left":
        return "right"
    if edge_label == "right":
        return "left"
...
def _profile_reference_from_banks(
    ...
    track_n: np.ndarray,
    track_epsg: int,
    start_edge_label: str | None = None,
    ...
    if not _PROJ_OK:
        if first_xy is None:
            raise RuntimeError("pyproj niet beschikbaar en geen geldige trackpunten om profiel lokaal te verankeren.")
        anchor_e = float(first_xy[0])
        anchor_n = float(first_xy[1])
        rel_e = te - anchor_e
        rel_n = tn - anchor_n
        ...
        if start_edge_label == "right":
            return (
                anchor_e,
                anchor_n,
                float((left_to_right + 180.0) % 360.0),
                "bestand start op rechteroever -> profiel rechteroever -> linkeroever "
                "(pyproj fallback: bestaand trackstartpunt behouden)",
            )
        if start_edge_label == "left":
            return (
                anchor_e,
                anchor_n,
                float(left_to_right),
                "bestand start op linkeroever -> profiel linkeroever -> rechteroever "
                "(pyproj fallback: bestaand trackstartpunt behouden)",
            )

        candidates = [
            (float(left_to_right), "fallback linkeroever -> rechteroever"),
            (float((left_to_right + 180.0) % 360.0), "fallback rechteroever -> linkeroever"),
        ]
        if np.isfinite(measured):
            chosen_bearing, chosen_label = min(candidates, key=lambda item: abs(_angle_diff_deg(item[0], measured)))
        else:
            chosen_bearing, chosen_label = candidates[0]
        return (
            anchor_e,
            anchor_n,
            chosen_bearing,
            f"{chosen_label} (pyproj fallback: bestaand trackstartpunt behouden)",
        )
    ...
    if not np.isfinite(measured):
        measured = _track_axis_bearing(rel_e, rel_n)
    candidates = [
        (float(left_to_right), float(left_e), float(left_n), "fallback linkeroever -> rechteroever"),
        (float((left_to_right + 180.0) % 360.0), float(right_e), float(right_n), "fallback rechteroever -> linkeroever"),
    ]
    if np.isfinite(measured):
...
def _align_track_to_profile(
    ...
    if allow_reverse:
        ...
        candidates = [float(target_bearing_deg)]
    chosen = min(candidates, key=lambda cand: abs(_angle_diff_deg(cand, measured)))
    delta = _angle_diff_deg(chosen, measured)
    rel_e, rel_n = _rotate_points(rel_e, rel_n, -delta)
    out_e = rel_e + float(anchor_e)
    ...
    if (not allow_reverse) and abs(abs(delta) - 180.0) <= 1.0:
        report.append("Raai omgedraaid zodat de richting overeenkomt met linkeroever -> rechteroever.")
    return out_e, out_n, report
...
def read_mat_3d(
    ...
    if mode == "dwarsprofiel":
        ...
        start_edge_label = _mat_start_edge_label(mat_path)
        anchor_e, anchor_n, target_bearing, chosen_label = _profile_reference_from_banks(
            ...
            track_epsg=track_epsg,
            start_edge_label=start_edge_label,
        )
        ...
        end_edge_label = _opposite_edge_label(start_edge_label)
        profile_reversed = start_edge_label == "right"
        if start_edge_label is not None:
            extra = [
                f"Startoever bestand: {start_edge_label}.",
                f"Eindoever bestand: {end_edge_label if end_edge_label is not None else 'n/a'}.",
                f"Raai omgedraaid: {'ja' if profile_reversed else 'nee'}.",
                f"Profielmodus: {chosen_label}.",
            ] + profile_report
        else:
...
def build_mat_preview_summary(
    ...
    start_edge_label = _mat_start_edge_label(mat_path) or "n/a"
    end_edge_label = _opposite_edge_label(start_edge_label) or "n/a"
...
def _write_processed_track_to_mat(
    src_mat_path: str | Path,
    out_mat_path: str | Path,
    processed_track_e: np.ndarray,
    processed_track_n: np.ndarray,
    track_is_ne: bool = False,
) -> Path:
    src = Path(src_mat_path)
    outp = Path(out_mat_path)
    mat = sio.loadmat(str(src), squeeze_me=True, struct_as_record=False)
    if "Summary" not in mat:
        raise ValueError(f"{src.name}: structuur 'Summary' ontbreekt.")
    sum_obj = mat["Summary"]
    if not hasattr(sum_obj, "Track"):
        raise ValueError(f"{src.name}: Summary.Track ontbreekt.")

    track = np.asarray(sum_obj.Track)
    if track.ndim != 2 or track.shape[1] < 2:
        raise ValueError(f"{src.name}: Summary.Track vorm onverwacht {track.shape}")

    track_out = np.array(track, copy=True)
    ns = min(track_out.shape[0], len(processed_track_e), len(processed_track_n))
    if ns <= 0:
        raise ValueError(f"{src.name}: geen geldige geproceste trackpunten om weg te schrijven.")

    if track_is_ne:
        track_out[:ns, 0] = np.asarray(processed_track_n[:ns], dtype=track_out.dtype)
        track_out[:ns, 1] = np.asarray(processed_track_e[:ns], dtype=track_out.dtype)
    else:
        track_out[:ns, 0] = np.asarray(processed_track_e[:ns], dtype=track_out.dtype)
        track_out[:ns, 1] = np.asarray(processed_track_n[:ns], dtype=track_out.dtype)

    sum_obj.Track = track_out
    payload = {k: v for k, v in mat.items() if not str(k).startswith("__")}
    outp.parent.mkdir(parents=True, exist_ok=True)
    sio.savemat(str(outp), payload, do_compression=False)
    return outp


def export_processed_mat_files(
    ...
    for src in [Path(p) for p in mat_paths]:
        data_full = read_mat_3d(
            mat_path=src,
            m9_hour_shift=m9_hour_shift,
            track_is_ne=track_is_ne,
            ensemble_step=1,
        ...
        base_name = f"{src.stem}_processed.mat"
        idx = used_names.get(base_name, 0)
        used_names[base_name] = idx + 1
        out_name = base_name if idx == 0 else f"{src.stem}_{idx:02d}_processed.mat"
        out_path = out_root / out_name
        outputs.append(
            _write_processed_track_to_mat(
                src_mat_path=src,
                out_mat_path=out_path,
                processed_track_e=data_full.track_e,
                processed_track_n=data_full.track_n,
                track_is_ne=track_is_ne,
            )
        )
    return outputs
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.var_make_processed_mat = tk.BooleanVar(value=True)
        ...
        self._preview_file_listbox: tk.Listbox | None = None
        self._loaded_preview: list[MatPreviewSummary] = []
        self._loaded_preview_all: list[MatPreviewSummary] = []
        self._loaded_signature: tuple[object, ...] | None = None
        self._suspend_selection_invalidate = False
    ...
    def _build(self) -> None:
        ...
        ttk.Button(btn_row, text="Selecteer alles", command=self._select_all_files).pack(side="left", padx=(8, 0))
        ttk.Button(btn_row, text="Deselecteer alles", command=self._deselect_all_files).pack(side="left", padx=(8, 0))
        ...
        self.listbox.bind("<<ListboxSelect>>", self._on_file_selection_changed)
        ...
        ttk.Checkbutton(opt, text="Maak geproceste MAT export", variable=self.var_make_processed_mat).grid(
            row=5, column=2, columnspan=3, sticky="w", pady=(8, 0)
        )
    ...
    def _on_file_selection_changed(self, _event: object | None = None) -> None:
        if self._suspend_selection_invalidate:
            return
        self._invalidate_loaded_data(close_preview=False)

    def _selected_indices(self) -> list[int]:
        return [int(i) for i in self.listbox.curselection()]

    def _selected_mat_files(self) -> list[Path]:
        idxs = self._selected_indices()
        return [self._mat_files[i] for i in idxs if 0 <= i < len(self._mat_files)]

    def _active_mat_files(self, require_selection: bool = False) -> list[Path]:
        files = self._selected_mat_files()
        if require_selection and not files:
            raise ValueError("Selecteer minstens één MAT-bestand in de lijst.")
        return files

    def _current_load_signature(self) -> tuple[object, ...]:
        return (
            tuple(self._path_key(p) for p in self._selected_mat_files()),
    ...
    def _sync_main_selection_from_paths(self, selected_keys: set[str]) -> None:
        self._suspend_selection_invalidate = True
        try:
            self.listbox.selection_clear(0, "end")
            for i, p in enumerate(self._mat_files):
                if self._path_key(p) in selected_keys:
                    self.listbox.select_set(i)
        finally:
            self._suspend_selection_invalidate = False

    def _preview_select_all(self) -> None:
        if self._preview_file_listbox is None:
            return
        self._preview_file_listbox.select_set(0, "end")

    def _preview_deselect_all(self) -> None:
        if self._preview_file_listbox is None:
            return
        self._preview_file_listbox.selection_clear(0, "end")

    def _apply_preview_selection(self) -> None:
        if self._preview_file_listbox is None:
            return
        sel = [int(i) for i in self._preview_file_listbox.curselection()]
        if not sel:
            messagebox.showerror("Selectie", "Selecteer minstens één bestand in het previewvenster.")
            return
        selected_previews = [self._loaded_preview_all[i] for i in sel if 0 <= i < len(self._loaded_preview_all)]
        selected_keys = {self._path_key(item.path) for item in selected_previews}
        self._sync_main_selection_from_paths(selected_keys)
        self._loaded_preview = selected_previews
        self._loaded_signature = self._current_load_signature()
        self._set_process_enabled(True)
        self._show_preview_window(self._loaded_preview_all, selected_keys)

    def _show_preview_window(
        self,
        previews_all: list[MatPreviewSummary],
        selected_keys: set[str] | None = None,
    ) -> None:
        self._close_preview_window()
        if selected_keys is None:
            selected_keys = {self._path_key(item.path) for item in previews_all}
        previews = [item for item in previews_all if self._path_key(item.path) in selected_keys]
        if not previews:
            raise ValueError("Geen previewbestanden geselecteerd.")
        ...
        selector_box = ttk.LabelFrame(outer, text="Bestandsselectie", padding=8)
        selector_box.grid(row=0, column=0, sticky="nsw", padx=(0, 10))
        selector_box.grid_rowconfigure(1, weight=1)
        selector_box.grid_columnconfigure(0, weight=1)

        selector_btns = ttk.Frame(selector_box)
        selector_btns.grid(row=0, column=0, sticky="we", pady=(0, 8))
        ttk.Button(selector_btns, text="Selecteer alles", command=self._preview_select_all).pack(side="left")
        ttk.Button(selector_btns, text="Deselecteer alles", command=self._preview_deselect_all).pack(side="left", padx=(6, 0))
        ttk.Button(selector_btns, text="Pas selectie toe", command=self._apply_preview_selection).pack(side="left", padx=(6, 0))

        preview_list = tk.Listbox(selector_box, height=max(8, min(20, len(previews_all))), selectmode=tk.EXTENDED, exportselection=False)
        preview_list.grid(row=1, column=0, sticky="nsew")
        self._preview_file_listbox = preview_list
        preview_scroll = ttk.Scrollbar(selector_box, orient="vertical", command=preview_list.yview)
        preview_scroll.grid(row=1, column=1, sticky="ns")
        preview_list.configure(yscrollcommand=preview_scroll.set)
        selected_keys_local = {self._path_key(item.path) for item in previews}
        for i, item in enumerate(previews_all):
            preview_list.insert("end", item.path.name)
            if self._path_key(item.path) in selected_keys_local:
                preview_list.select_set(i)
        ...
        cols = ("bestand", "startoever", "eindoever", "omgedraaid", "start", "stop", "vmin", "vgem", "vmax", "debiet")
        tree = ttk.Treeview(table_box, columns=cols, show="headings", height=max(6, min(18, len(previews) + 1)))
        headers = {
            "bestand": "Bestand",
            "startoever": "Startoever",
            "eindoever": "Eindoever",
            "omgedraaid": "Raai omgedraaid",
            "start": "Start",
            "stop": "Stop",
            "vmin": "V min",
            "vgem": "V gem",
        ...
        for col in cols:
            ...
            tree.column(
                ...
                width=widths[col],
                anchor=anchors[col],
                stretch=(col in {"bestand", "start", "stop"}),
            )
        for item in previews:
            tree.insert(
                ...
                values=(
                    item.path.name,
                    item.start_edge_label,
                    item.end_edge_label,
                    ("ja" if item.profile_reversed else "nee"),
        ...
        ttk.Label(
            ...
            justify="left",
        ).grid(row=1, column=0, columnspan=2, sticky="we", pady=(8, 0))
    ...
    def _load_data(self) -> None:
        try:
            if not self._mat_files:
                ...
                return
            active_files = self._active_mat_files(require_selection=True)
            ...
            failed: list[str] = []
            self._log(f"MAT-bestanden inlezen voor preview... selectie={len(active_files)} bestand(en)")
            ...
            self._loaded_preview_all = previews
    ...
    def _refresh_listbox(self, select_all: bool = True) -> None:
        self.listbox.delete(0, "end")
        for p in self._mat_files:
            self.listbox.insert("end", str(p))
        if select_all and self._mat_files:
    ...
    def _process_data(self) -> None:
        try:
            ...
            make_processed_mat = bool(self.var_make_processed_mat.get())
            ...
            self._log(f"MAT-bestanden verwerken... selectie={len(active_files)} bestand(en)")
            ...
            processed_mat_paths: list[Path] = []
            if make_processed_mat:
                processed_mat_dir = outp.parent / f"{outp.stem}_processed_mat"
                self._log("Geproceste MAT export bouwen...")
                processed_mat_paths = export_processed_mat_files(
                    mat_paths=active_files,
                    out_dir=processed_mat_dir,
                    m9_hour_shift=m9_shift,
                    track_is_ne=track_is_ne,
                    measurement_mode=measurement_mode,
                    reference_path=reference_kmz,
                ...
                self._log(f"Geproceste MAT map: {processed_mat_dir}")
                for mp in processed_mat_paths:
                    self._log(f"MAT: {mp}")
