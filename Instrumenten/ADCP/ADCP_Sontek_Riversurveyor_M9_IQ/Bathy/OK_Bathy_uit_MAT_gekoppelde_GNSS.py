...
- Bodemdiepte altijd via BT_Depth voor alle ingelezen .mat-bestanden.
- DGPS (UTM of Lat/Lonâ†’UTM) automatisch; indien geen DGPS: georefereer Summary.Track met UTM-anker.
...
- Dieptefilter: verwijdert meetpunten buiten [MinDiepte, MaxDiepte] (m), zodat foutieve â€œhoogtesâ€ of absurd diepe outliers verdwijnen.
...
_depth_choice_global = 'bt'  # 'vb' of 'bt' â€” Ã©Ã©nmalig per sessie

def extract_depth(data):
    """Gebruik altijd BottomTrack.BT_Depth; pak tijd uit Summary/BottomTrack indien aanwezig."""
    ...
    if bt_val is None:
        raise KeyError("Geen BT_Depth gevonden in 'BottomTrack'.")

    Z = _as_array(bt_val).astype('float64')
    dtype = 'BT_Depth'
...
def extract_dgps_xy(data, epsg_pref=32631, allow_auto_zone=True):
    ...
    for c in containers:
        ...
        if LA is not None and LO is not None:
            ...
            return X, Y, t_gps, f"DGPS Lat/Lonâ†’UTM (EPSG:{used_epsg}) '{kla}/{klo}'"
...
def georef_track_with_anchor(data, default_e=650000.0, default_n=5650000.0):
    ...
    if tr is None or tr.ndim != 2 or tr.shape[1] < 2:
        raise KeyError("Summary.Track heeft geen 2 kolommen (Î”E, Î”N).")
...
def align_xy_to_depth_time(X, Y, t_gps, t_depth, tol_seconds=2.0):
    ...
    # Zonder tijden maar gelijke lengte â†’ directe map (typisch bij Track)
...
class BathyModel:
    def interpolate(self, X, Y, Z, grid_res=150):
        ...
        raise ValueError("XY is (bijna) eendimensionaal â€“ overschakelen naar 1D-profiel.")
...
class BathyProfileGUI:
    ...
    def load_and_plot(self):
        ...
        for f in files:
            try:
                ...
                # Diepte en tijd (Ã©Ã©nmalige keuze wordt intern onthouden)
    ...
    def _recompute_and_plot(self):
        ...
        for X, Y, Z in zip(self.raw_X, self.raw_Y, self.raw_Z):
            # dieptefilter vÃ³Ã³r spikefilter (om absurde waarden meteen te droppen)
