...
def align_xy_to_depth_time(X, Y, t_gps, t_depth, tol_seconds=2.0):
    ...
    # Zonder tijden maar gelijke lengte â†’ directe map
...
def _nearly_collinear(X, Y, eps_rel=1e-6):
    ...
    if w[-1] == 0:
        return True
    ratio = w[0] / w[-1]
    return ratio < eps_rel  # heel kleine tweede eigenwaarde â†’ quasi-lijn
...
class BathyModel:
    def interpolate(self, X, Y, Z, grid_res=150):
        ...
        if not collinear and (xspan > 0) and (yspan > 0):
            # 2D mogelijk â†’ probeeer cubic/linear, met jitter fallback
        ...
        # 1D-profiel fallback
        raise ValueError("XY is (bijna) Ã©Ã©ndimensionaal â€“ overschakelen naar 1D-profiel.")
