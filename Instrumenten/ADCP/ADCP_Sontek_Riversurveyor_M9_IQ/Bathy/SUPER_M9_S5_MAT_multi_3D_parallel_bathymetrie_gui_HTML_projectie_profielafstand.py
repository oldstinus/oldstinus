...
In deze variant wordt de profielafstand voor het gemiddeld profiel berekend als
een projectie op een rechte profielas, niet als cumulatieve afgelegde weg.
"""
...
def _projected_chainage_from_track(track_e: np.ndarray, track_n: np.ndarray) -> np.ndarray:
    te = np.asarray(track_e, dtype=float).reshape(-1)
    tn = np.asarray(track_n, dtype=float).reshape(-1)
    valid = np.isfinite(te) & np.isfinite(tn)
    ...
    origin_e, origin_n, dir_vec, _ = _reference_axis(te, tn)
    rel_e = te - float(origin_e)
    rel_n = tn - float(origin_n)
    s = rel_e * float(dir_vec[0]) + rel_n * float(dir_vec[1])
    s[~valid] = np.nan
    return s
...
def _profile_points(data: BathyTrackData) -> tuple[np.ndarray, np.ndarray]:
    s = _projected_chainage_from_track(data.track_e, data.track_n)
...
def build_mean_bathy_figure(
    ...
    fig.update_layout(
        ...
        xaxis_title="Geprojecteerde profielafstand vanaf start (m)",
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("M9 MAT multi 3D parallelle bathymetrie (geprojecteerde profielafstand)")
        self.geometry("1140x820")

        self.var_out = tk.StringVar(value=str(Path.cwd() / "m9_parallel_bathymetrie_3d_projectie_profielafstand.html"))
    ...
    def _build(self) -> None:
        ...
        ttk.Label(opt, text="BottomTrack.BT_Depth").grid(row=1, column=1, columnspan=2, sticky="w", pady=(8, 0))
    ...
    def _run(self) -> None:
        try:
            ...
            add_interactive_html_saver(mean_html)
