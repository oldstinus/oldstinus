...
def transform_velocities(data_filtered, transformation_matrix):
    """Transformeert beam-snelheden naar ENU-coördinaten voor Vector data."""
    ...
    data_filtered = data_filtered.assign(
        Velocity_East=V_ENU[:, 0],
        Velocity_North=V_ENU[:, 1],
        Velocity_Up=V_ENU[:, 2]
    )
...
def calculate_resultant_speed_direction(data_filtered):
    ...
    data_filtered['Direction'] = (np.degrees(np.arctan2(data_filtered['Velocity_North'], data_filtered['Velocity_East'])) + 360) % 360
    return data_filtered
...
def load_vector_data(dat_file_path, sen_file_path, dat_column_names, sen_column_names, start_datetime, end_datetime):
    ...
    return data_filtered, transformation_matrix, fixed_vector_depth
...
def main():
    ...
    root.geometry("900x600")  # Vergroot de grootte indien nodig
