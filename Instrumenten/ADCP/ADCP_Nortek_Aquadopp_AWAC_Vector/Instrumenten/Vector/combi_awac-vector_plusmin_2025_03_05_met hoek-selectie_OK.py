...
def load_vector_data(dat_file, sen_file):
    ...
    # Bereken de resulterende snelheid en de originele richting
...
def calculate_vector_signed_speed(df_vector, pos_range, neg_range):
    # Bepaal het teken op basis van de originele Direction
    def get_sign(angle):
...
def process_awac_data_by_cell(df_awac, pos_min, pos_max, cell_key, cells):
    ...
    # Indien er aparte u- en v-componenten beschikbaar zijn, kan hier de richting herberekend worden.
    direction_data = df_awac[d_col].astype(float)
    df_awac_processed = df_awac.copy()
    df_awac_processed['signed_speed'] = signed_speed(speed_data, direction_data, pos_min, pos_max)
    df_awac_processed['Speed'] = speed_data
    df_awac_processed['Direction'] = direction_data
...
class SensorComparisonApp:
    def __init__(self, root):
        ...
        # Tijd (in eerste instantie handmatig, maar na data laden wordt de overlappende periode ingesteld)
    ...
    # --- Data laden ---
    def load_data(self):
        # Eerst de data laden zoals eerder
        try:
        ...
        # Bepaal de tijdsinterval per dataset
        vector_start = df_vector['Datetime'].min()
        vector_end = df_vector['Datetime'].max()
        awac_start = df_awac['DateTime'].min()
        awac_end = df_awac['DateTime'].max()

        # Bereken de overlappende periode
        start_overlap = max(vector_start, awac_start)
        end_overlap = min(vector_end, awac_end)

        if start_overlap >= end_overlap:
            messagebox.showerror("Fout", "Er is geen overlappende periode tussen de twee datasets.")
            return

        # Update de GUI-velden voor start- en einddatum met de overlappende periode
        self.start_datetime_var.set(start_overlap.strftime("%Y-%m-%d %H:%M:%S"))
        self.end_datetime_var.set(end_overlap.strftime("%Y-%m-%d %H:%M:%S"))

        # Filter de data op de overlappende periode
        df_vector = df_vector[(df_vector['Datetime'] >= start_overlap) & (df_vector['Datetime'] <= end_overlap)].copy()
        df_awac = df_awac[(df_awac['DateTime'] >= start_overlap) & (df_awac['DateTime'] <= end_overlap)].copy()
        ...
        try:
            ...
            messagebox.showerror("Fout", f"Fout in de snelheidsranges: {e}")
            return
        try:
            ...
            messagebox.showerror("Fout", f"Fout bij het verwerken van vector data: {e}")
            return
        ...
        try:
            ...
            messagebox.showerror("Fout", f"Fout bij het verwerken van AWAC cellen: {e}")
            return
        ...
        messagebox.showinfo("Info", "Data succesvol geladen.\nOverlappende periode: {} tot {}".format(
            start_overlap.strftime("%Y-%m-%d %H:%M:%S"), end_overlap.strftime("%Y-%m-%d %H:%M:%S")
        ))
    ...
    def generate_graph(self):
        ...
        # Gemeenschappelijke tijdsreeks (gebaseerd op de overlappende periode)
        start_common = df_vector['Datetime'].min()
        end_common = df_vector['Datetime'].max()
        ...
        fig.suptitle("Vergelijking van Sensor Data (zonder extra hoek-shift)", fontsize=16)

        # (0,0): Lijn: Vector snelheid (magnitude)
        ...
        # (0,1): Lijn: AWAC snelheid (magnitude)
        ...
        # (0,2): Gecombineerde lijngrafiek: Originele snelheden van beide sensoren (zonder sign aanpassing)
        ...
        # (1,0): Lijn: Vector signed snelheden over tijd
        ...
        # (1,1): Lijn: AWAC signed snelheden over tijd (origineel)
        ...
        # (1,2): Lijn: Vergelijking signed snelheden met optimale tijdshift
        ...
        r_val = np.corrcoef(df_vector_signed['Velocity_Channel_avg'], df_awac_signed_shifted['signed_speed_avg'])[0,1]
        r2_val = r_val**2
        ...
        # (2,0): Lijn: Vector richting (origineel)
        ...
        # (2,1): Lijn: AWAC richting (origineel)
        ...
        # (2,2): Scatter: Vergelijking X-Y van signed snelheden (met plus/min teken)
        ...
        axs[2,2].plot([df_vector_signed['Velocity_Channel_avg'].min(), df_vector_signed['Velocity_Channel_avg'].max()],
                       ...
                       'k--', label="1:1 lijn")
        r_scatter = np.corrcoef(df_vector_signed['Velocity_Channel_avg'], df_awac_signed['signed_speed_avg'])[0,1]
        axs[2,2].set_title("Scatter Signed Snelheden (Plus/Min)\n(R² = {:.3f})".format(r_scatter**2))
        axs[2,2].set_xlabel("Vector Signed (m/s)")
        axs[2,2].set_ylabel("AWAC Signed (m/s)")
        axs[2,2].legend()
        axs[2,2].grid(True)

        # Zorg dat de tijd-as op de relevante plots leesbaar is
        for ax in [axs[1,2]]:
