...
def choose_time_window_on_pressure(
    time_index: pd.DatetimeIndex,
    pressure: np.ndarray,
) -> tuple[pd.Timestamp, pd.Timestamp]:
    times = pd.DatetimeIndex(pd.to_datetime(time_index))
    if len(times) == 0:
        raise ValueError("Geen tijdstappen beschikbaar voor drukselectie.")

    pressure_arr = np.asarray(pressure, dtype=float)
    if pressure_arr.shape[0] != len(times):
        raise ValueError("Pressure-lengte komt niet overeen met de tijdas.")

    fig, ax = plt.subplots(1, 1, figsize=(15, 6))
    ax.plot(times, pressure_arr, color="black", linewidth=1.0, label="druk")
    ax.set_title("Selecteer tijdvenster op de drukgrafiek")
    ...
    span_patch = [ax.axvspan(times[0], times[-1], color="tab:blue", alpha=0.12)]
    ...
    help_text = fig.text(
        0.98,
        0.02,
        "Sleep over de drukgrafiek om start/stop te kiezen. Enter = bevestigen, r = reset.",
        ha="right",
        va="bottom",
        fontsize=10,
    )
    ...
    def update_status() -> None:
        if span_patch[0] is not None:
            span_patch[0].remove()
        span_patch[0] = ax.axvspan(selection["start"], selection["end"], color="tab:blue", alpha=0.12)
        status_text.set_text(
            f"Selectie: {selection['start']:%Y-%m-%d %H:%M:%S} -> {selection['end']:%Y-%m-%d %H:%M:%S}"
        )
        fig.canvas.draw_idle()

    def on_select(xmin: float, xmax: float) -> None:
        start_num, end_num = sorted((xmin, xmax))
        start_ts = pd.Timestamp(mdates.num2date(start_num)).tz_localize(None)
        end_ts = pd.Timestamp(mdates.num2date(end_num)).tz_localize(None)
        selection["start"] = max(start_ts, pd.Timestamp(times[0]))
        selection["end"] = min(end_ts, pd.Timestamp(times[-1]))
    ...
    def on_key(event) -> None:
        if event.key == "enter":
            ...
            selection["start"] = pd.Timestamp(times[0])
            selection["end"] = pd.Timestamp(times[-1])
...
def apply_time_window(
...
) -> tuple[pd.DatetimeIndex, np.ndarray, np.ndarray, np.ndarray, pd.DataFrame]:
    times = pd.DatetimeIndex(pd.to_datetime(time_index))
    ...
    if not np.any(mask):
        raise ValueError("De gekozen tijdselectie bevat geen meetpunten.")

    filtered_time = times[mask]
    filtered_orientation = orientation.iloc[np.where(mask)[0]].reset_index(drop=True)
    filtered_orientation.index = filtered_time
    return (
        filtered_time,
        east[mask, :],
        north[mask, :],
...
def main() -> None:
    ...
    selected_start, selected_end = choose_time_window_on_pressure(
        time_index,
        orientation["pressure"].to_numpy(dtype=float),
    )
    time_index, east, north, up, orientation = apply_time_window(
        time_index,
        east,
        north,
        up,
    ...
    print(f"Geselecteerd tijdvenster: {time_index[0]} -> {time_index[-1]}")
