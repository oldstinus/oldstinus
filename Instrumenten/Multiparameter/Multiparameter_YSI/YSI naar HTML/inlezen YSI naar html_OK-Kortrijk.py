...
def main():
    ...
    # --- Voeg scrollable functionaliteit toe ---
    ...
    # Maak een hoofdframe binnen de canvas waarin alle widgets worden geplaatst
    main_frame = tk.Frame(canvas)
    canvas.create_window((0, 0), window=main_frame, anchor="nw")

    # Zorg dat de scrollregion wordt aangepast wanneer de grootte van het frame verandert
    def on_configure(event):
        canvas.configure(scrollregion=canvas.bbox("all"))
    main_frame.bind("<Configure>", on_configure)
    # --- Einde scrollable functionaliteit ---
    ...
    project_number_var = tk.StringVar()  # Variabele voor projectnummer
    ...
    def generate_plot():
        ...
        try:
            ...
            safe_location_name = re.sub(r'[^\w\-_\. ]', '_', location_name.strip())[:50]
    ...
    # ----------------------------
    # GUI-elementen worden toegevoegd aan het main_frame in plaats van root
    # Titelkader (Title Frame)
    title_frame = tk.Frame(main_frame, bd=2, relief=tk.RIDGE, padx=10, pady=10)
    ...
    # Delimiter selectie
    delimiter_frame = tk.Frame(main_frame)
    ...
    decimal_frame = tk.Frame(main_frame)
    ...
    # Tijdkolom selectie
    time_column_frame = tk.Frame(main_frame)
    ...
    start_datetime_frame = tk.Frame(main_frame)
    ...
    end_datetime_frame = tk.Frame(main_frame)
    ...
    column_frame = tk.LabelFrame(main_frame, text="Selecteer kolommen en assen:")
    column_frame.pack(pady=5, fill="both", expand="yes")
    # Widgets worden dynamisch toegevoegd in load_columns()

    # Preview Frame voor Data
    preview_frame = tk.LabelFrame(main_frame, text="Data Preview (Eerste 10 Rijen):")
    preview_frame.pack(pady=5, fill="both", expand="yes")

    # Plot knop
    plot_button = tk.Button(main_frame, text="Genereer Grafiek", command=generate_plot)
