...
"""
#******************************************************************************
# This program computes the wave characteristics of the short wave surface 
...
#year='2022'
#month='1'
#day_start,day_end = '01','01'
#start_time = year+'-'+month+'-09 00:00:00' 
#end_time= year+'-'+month+'-12 00:00:00'  
output_suffix=''
...
basedir = r'P:\PA029-BMeetinstr-Cmp\3_Uitvoering\0_03_MEETCAMPAGNES_CALIBRATION_MEETNET\2024_10_22_Test_drukkamer_HF-druksensor_Diver\HF-druksensor'
...
datafile_eta = os.path.join(basedir,'druksensoren','eta',location+'_eta_SW_'+'%i' %fs +'Hz_'+st.strftime('%Y_%m_%d')+'_'+te.strftime('%Y_%m_%d')+output_suffix+ '.dat')
fileT = open(datafile_eta, 'r')
eta = pd.read_csv(datafile_eta,header=None)
eta_np=eta.iloc[:,0:].values.flatten()

H_times = pd.date_range(pd.to_datetime(start_time),pd.to_datetime(start_time)+(len(eta_np)-1)*pd.Timedelta(0.5,'s'),freq='500L')
eta=pd.DataFrame({'eta':eta_np},index=H_times)
...
M = 128*fs                              #segment length for spectral averaging in short wave analysis (Table 3-2 IMDC report RA15039) 
p = 13                                  #number of segments (Table 3-2 IMDC report RA15039) 
...
for i in range(0,Nwindows):
    ...
    Noverlap = math.floor((m-p*M)/(1-p))  #length of overlap between segments (adjusted to length of window without NaN values)
    p_check = p
    count = 0
...
#        if m_star < m: 
    ...
    if st_Sf_out <= time <= et_Sf_out:
        ...
        data_Sf.to_csv(os.path.join(Spectrum_folder,'SW_Sf_'+location+'_'+datetime.strftime(time,'%d-%m-%Y %H-%M')+'.txt'),sep='\t',header=True,index=None)
...
spectrum_out.to_csv(os.path.join(Spectrum_folder,'SW_Hm0_Tm_10_'+location+'_15min.txt'),header=True,sep='\t',float_format = '%.3e')
